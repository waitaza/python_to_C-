...
// 计算两个日期之间的月数和年数
static PyObject* calculate_months_years_diff(PyObject* self, PyObject* args) {
    const char* date_str1;
    const char* date_str2;
    struct tm tm1, tm2;
    int years, months;
    ...
    return Py_BuildValue("ii", years, months);
}

// 获取月份的天数
static PyObject* get_days_in_month(PyObject* self, PyObject* args) {
    int year, month;
    struct tm tm = {0};
    ...
    // Move to the next month and backtrack one day to get the last day of the current month
    tm.tm_mday = 1;
    tm.tm_mon += 1;
    ...
    return Py_BuildValue("i", tm.tm_mday);
}

// 将日期格式转换为不同的格式
static PyObject* convert_date_format(PyObject* self, PyObject* args) {
    const char* date_str;
    ...
    if (!PyArg_ParseTuple(args, "sss", &date_str, &from_format, &to_format)) {
        return NULL;
    }

    memset(&tm, 0, sizeof(struct tm));

    if (strptime(date_str, from_format, &tm) == NULL) {
        PyErr_SetString(PyExc_ValueError, "Error parsing date");
        return NULL;
    }

    if (strftime(buffer, sizeof(buffer), to_format, &tm) == 0) {
        PyErr_SetString(PyExc_ValueError, "Error formatting date");
        return NULL;
    }
...
// 获取某个日期的星期几
static PyObject* get_weekday(PyObject* self, PyObject* args) {
    const char* date_str;
    struct tm tm;
    ...
    mktime(&tm);
    return Py_BuildValue("i", tm.tm_wday);
}

// 计算某个日期是今年的第几周
static PyObject* get_week_of_year(PyObject* self, PyObject* args) {
    const char* date_str;
    struct tm tm;
    char buffer[256];
    ...
    if (strftime(buffer, sizeof(buffer), "%U", &tm) == 0) {
        PyErr_SetString(PyExc_ValueError, "Error formatting date");
        return NULL;
    }

    return Py_BuildValue("i", atoi(buffer));
}
...
static PyMethodDef DateUtilMethods[] = {
    ...
    {"calculate_months_years_diff", calculate_months_years_diff, METH_VARARGS, "Calculate the difference in months and years between two dates"},
    {"get_days_in_month", get_days_in_month, METH_VARARGS, "Get the number of days in a month"},
    {"convert_date_format", convert_date_format, METH_VARARGS, "Convert a date format to another format"},
    {"get_weekday", get_weekday, METH_VARARGS, "Get the weekday of a date"},
    {"get_week_of_year", get_week_of_year, METH_VARARGS, "Get the week number of the year for a date"},
    {NULL, NULL, 0, NULL}
};
